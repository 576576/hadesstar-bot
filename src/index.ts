...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.command('CK [arg]', '查询组队情况 例: CK CK9')
    ...
    .action(async ({ session }, arg) => {
      await sleep(Math.random() * 1000)
      if (isValidDrsNum(+arg)) {
        await session.onebot?.sendGroupMsg(session.guildId, await showALine(session, +arg))
      }
      else await session.onebot?.sendGroupMsg(session.guildId, await showAllLines(session))
    })
  ...
  ctx.command('CX [arg]')
    .action(async ({ session }, arg) => {
      ...
      if (tmp != null) {
        await sleep(Math.random() * 1000)
        await session.onebot?.sendGroupMsg(session.guildId, await formatted_playerdata(session, tmp))
      }
    })
  ...
  ctx.command('LR <arg>', 'LR 创0富0延0强0')
    .action(async ({ session }, arg) => {
      ...
      if (techs_in != null) {
        ...
        await session.onebot?.sendGroupMsg(session.guildId, `已录入${await getTech(getQQid(session))}`)
      }
      else {
    ...
    .action(async ({ session }, arg, arg2) => {
      ...
      if (arg2 == undefined) return
        ...
        if (playerGroup != '') {
          ...
          await session.onebot?.sendGroupMsg(session.guildId, `已录入常驻集团 ${await getGroup(tmp)}`)
        }
      }
    })
  ...
  ctx.command('LRHH <arg> <arg2>')
    .action(async ({ session }, arg, arg2) => {
      ...
      let einfo = await updateEventScore(session, +arg, +arg2)
      if (einfo != null) {
  ...
  ctx.command('CXHH [arg]')
    .action(async ({ session }, arg) => {
      ...
      let einfo = await getEventInfo(tmp)
  ...
  async function quit_drs(session: Session): Promise<void> {
    let foundType = await findDrsFromId(session, getQQid(session))
    if (foundType != 'K0') {
      await ctx.database.remove('dlines', { qid: getQQid(session) })
      await session.onebot?.sendGroupMsg(session.guildId, `${await getNameFromQid(session, getQQid(session))} 已退出${foundType}队列`)
    }
  ...
  async function join_rs_event(session: Session, joinType: string): Promise<number> {
    ...
    let driverLicence = await getLicence(getQQid(session))
    if (driverLicence < lineLevel) {
      await session.onebot?.sendGroupMsg(session.guildId, `你未获得${joinType}车牌`)
      return null
    }
  ...
  async function updateEventScore(session: Session, lineId_in: number, score: number, playerId?: number): Promise<number[]> {
  ...
  async function findIdFromDrs(checkType: string): Promise<number[]> {
  ...
  async function findDrsFromId(session: Session, playerId: number): Promise<string> {
  ...
  async function formatted_DrsN(session: Session, targetType: string): Promise<string> {
  ...
  async function formatted_RsEvent(session: Session, playerId: number, isDetail?: boolean): Promise<string> {
    let playerName = await getNameFromQid(session, playerId)
    let playerRoute = await getPlayRoutes(playerId)
    let playerTech = await getTech(playerId)
    let playerGroup = await getGroup(playerId)
    let einfo = await getEventInfo(playerId)
    return isDetail ? `╔${await getNameFromQid(session, playerId)}:\n╠ 集团 [${playerGroup}]╠ 场次[${einfo[0]}]\n 总分 [${einfo[1]}]` :
      `${await getNameFromQid(session, playerId)}:\n 集团 [${playerGroup}]\n 次数 [${einfo[0]}]\n 总分 [${einfo[1]}]`
  }
  ...
  async function showAllLines(session: Session): Promise<string> {
    ...
    for (var i = 7; i <= 12; i++) {
      lineMsg = ''
      tmp = await formatted_DrsN(session, `D${i}`)
      if (tmp.indexOf('队列为空') != -1) lineMsg += `D${i}队列—————\n${tmp}${await drs_timer(`D${i}`)}\n`
      tmp = await formatted_DrsN(session, `K${i}`)
      if (tmp.indexOf('队列为空') != -1) lineMsg += `K${i}队列—————\n${tmp}${await drs_timer(`K${i}`)}\n`
      linesMsg += lineMsg
    }
  ...
  async function getLicence(playerId: number): Promise<number> {
    return (await ctx.database.get('players', { qid: playerId }, ['licence']))[0].licence
  }

  async function getPlayRoutes(playerId: number): Promise<number[]> {
    return (await ctx.database.get('players', { qid: playerId }, ['playRoutes']))[0].playRoutes
  }

  async function getTech(playerId: number): Promise<string> {
  ...
  async function getGroup(playerId: number): Promise<string> {
  ...
  async function getNameFromQid(session: Session, playerId: number): Promise<string> {
    let tmp: string
  ...
  async function formatted_playerdata(session: Session, playerId: number): Promise<string> {
    return `玩家: ${await getNameFromQid(session, playerId)}\n集团: ${await getGroup(playerId)}\n车牌: D${await getLicence(playerId)}\n场数: ${await getPlayRoutes(playerId)}\n科技: ${await getTech(playerId)}`
  }
